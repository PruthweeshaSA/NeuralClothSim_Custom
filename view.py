...
def main(config, loc=(0, 0, 0), rot=(0, 0, 0)):
    ...
    if garment.active_material is None:
        garment.data.materials.append(bpy.data.materials.new(name=f"{config.name}_mat"))
    ...
    if garment_unskinned.active_material is None:
        garment_unskinned.data.materials.append(bpy.data.materials.new(name=f"{config.name}_unskinned_mat"))
